...
for (j in 1:5) {
  if (j == 1) {
    ...
    u_y <- arima.sim(model = list(ar = 0.5), n = T, sd = 1)
    y2 <- 10 + 2.5 * x + u_y
  } else if (j == 3) {
...
#+++++++++++++++++++++++++++++++
y_limits <- range(c(x, y1, y2, y3, y4, y5))
...
legend(x = 205, y = max(y_limits), xpd = TRUE,
       legend=c("X (Random Walk)", "Very strong (AR=0)", "strong (AR=0.5)","Weak (AR=0.8)", "Very Weak (AR=0.95)", "None (AR=1.0)"),
       col=c("black", "blue", "green", "purple", "pink", "red"),
...
master_data <- data.frame(
  Time_Index = 1:T,
  X_Random_Walk = x,
  Y1_very_strong_Coint = y1,
  Y2_strong_Coint = y2,
  Y3_Weak_Coint = y3,
  Y4_Very_Weak_Coint = y4,
  Y5_No_Coint = y5
)
# Save the combined data to your hard drive
write.csv(master_data, file = "data(2).csv", row.names = FALSE)
